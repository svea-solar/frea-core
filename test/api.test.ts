import { createMod } from "../src/io/http/mod";
import { ModuleSchema, HttpMod } from "../src/io/http/types";
import axios from "axios";

const port = "5000";

const setupModule = (api: HttpMod) => {
  const moduleApi = {
...
test("schemas", async () => {
  const api = await createMod({ port, corsList: [`http://localhost:${port}`] });
  setupModule(api);
